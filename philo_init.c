...
/*   Updated: 2025/10/25 09:49:33 by jinwpark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	init_cleanup(t_info *info)
{
	if (info->lock)
	{
...
static int	allocate_resources(t_info *info)
{
...
int	info_init(t_info *info, char **argv, int argc)
{
	...
	if (info->philo_num <= 0 || info->time_die <= 0 || info->time_eat <= 0
		...
		return (1);
	info->is_end = 0;
	return (allocate_resources(info));
}

static void	init_philosopher(t_info *info, int i)
{
